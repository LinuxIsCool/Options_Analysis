...
try:
    ...
    raise SystemExit(
        (
            (
                f"Option data for {stock_of_interest} not found in path: "
                + os.path.abspath(
                    os.path.join(option_data_path, stock_of_interest)
                )
            )
            + ".csv"
        )
...
try:
    ...
    raise SystemExit(
        (
            (
                f"Daily closing price data for {stock_of_interest} not found in path: "
                + os.path.abspath(
                    os.path.join(stock_data_path, stock_of_interest)
                )
            )
            + ".csv"
        )
